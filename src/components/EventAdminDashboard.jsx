...
const EventAdminDashboard = () => {
  ...
  // Componente Event Selector (debajo del header) - Con búsqueda y scroll
  const EventSelector = () => (
    <Box sx={{
      width: '100%',
      bgcolor: '#F5F7FA',
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const ventasAlimentosBebestibles = (eventDetails.alimentosBebestibles || []).reduce((total, item) => {
          const stockAsignado = item.stockAsignado || 0;
          const stockActual = item.stockActual || stockAsignado; // Default to assigned if no actual
        ...
        console.log('📊 Sales Breakdown:', {
          ...
          alimentosBebestibles: eventDetails.alimentosBebestibles?.length || 0,
        ...
        const alimentosBebidasSummary = {
          vendidas: (eventDetails.alimentosBebestibles || []).reduce((total, item) => {
            const stockAsignado = item.stockAsignado || 0;
            const stockActual = item.stockActual || stockAsignado;
            return total + (stockAsignado - stockActual);
          }, 0),
          disponibles: (eventDetails.alimentosBebestibles || []).reduce((total, item) => {
            return total + (item.stockActual || 0);
          }, 0),
          valorUnitarioPromedio: (eventDetails.alimentosBebestibles || []).length > 0 ? 
            (eventDetails.alimentosBebestibles || []).reduce((total, item) => {
              return total + (parseFloat(item.precioUnitario) || 0);
            }, 0) / (eventDetails.alimentosBebestibles || []).length : 0,
        ...
        const actividadesSummary = {
          ...
          disponibles: (eventDetails.actividades || []).reduce((total, actividad) => {
            ...
            const cuposOcupados = actividad.cuposOcupados || 0;
            const disponiblesItem = cuposDisponibles - cuposOcupados;
            return total + disponiblesItem;
          }, 0),
           ...
           <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%', mr: 3 }}>
             ...
             <Paper sx={{ 
               ...
               <Box>
                 ...
                 {selectedEvent?.alimentosBebestibles && selectedEvent.alimentosBebestibles.length > 0 ? selectedEvent.alimentosBebestibles.map((item, index) => {
                   const stockAsignado = item.stockAsignado || 0;
